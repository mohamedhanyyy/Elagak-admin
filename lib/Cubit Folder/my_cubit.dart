...
import 'package:elagy_pharmacy_app/models/orders/one_order_model.dart';
...
import '../models/aboutUs_ContactUs/about_contact_us_model.dart';
import '../models/auth/login.dart';
import '../models/orders/finish_order_model.dart';
import '../models/orders/pharmacy_orders.dart';
import '../models/orders/report_order_model.dart';
import '../models/profile/add_medicine_models.dart';
import '../models/profile/all_medicines_model.dart';
import '../models/profile/edit_medicine.dart';
import '../models/profile/medicines.dart';
import '../models/profile/one_pharmacy_model.dart';
import '../models/profile/profile_medicines.dart' as prof;
import '../modules/Home/home_page.dart';
import '../modules/auth/login_pharmacy.dart';
import '../modules/darwerScreens/about_us.dart';
import '../modules/darwerScreens/complaints.dart';
import '../modules/darwerScreens/contact_us.dart';
import '../modules/darwerScreens/pharmacy_user_profile_screen.dart';
...
class PharmacyCubit extends Cubit<PharmacyStates> {
  ...
  Future<void> login({required String password, required String email, required BuildContext context}) async {
    loadingAuthScreens(true);
    emit(HomeRegisterLoadingState());
    await http.post(Uri.parse('${api}login/pharmacy'),
        body: {
          'email': email,
          'password': password,
      ...
      if (loginPharmacyModel!.code == 200) {
        CacheHelper.putData(key: "token", value: loginPharmacyModel!.data!.token.toString());
        CacheHelper.putData(key: "id", value: loginPharmacyModel!.data!.id.toString());
        if (kDebugMode) {
          print("**********************");
        }
        if (kDebugMode) {
        ...
        getCategories();
        if (kDebugMode) {
          print("getCategories from login Pharmacy Id is : ${loginPharmacyModel!
              .data!.id.toString()}");
        }
        if (kDebugMode) {
        ...
        getAllMedicines();
        if (kDebugMode) {
          print("getAllMedicines from login Pharmacy Id is : ${loginPharmacyModel!.data!.id.toString()}");
  ...
  Future<void> resetPassword(
    ...
    await http.post(
      ...
      if (jsonDecode(value.body)['code'] == 201) {
        ...
        navigateFinalTo(context, const LoginScreenPharm());
        loadingAuthScreens(false);
        emit(HomeRegisterSuccessState());
      } else {
  ...
  Future contactUS(
      {required String body, required String title, required BuildContext context}) async {
    loadingAuthScreens(true);
    emit(HomeRegisterLoadingState());
    await http.post(Uri.parse('${api}contact-us'), body: {
      ...
      if (jsonDecode(value.body)['code'] == 201) {
                ...
                child: SizedBox(
                  ...
                  child: Padding(
                    ...
                    child: Column(
                      ...
                      children: [
                        myImage('assets/images/Login3/3elagk WORD.png'),
                        const Text(
        ...
        navigateFinalTo(context, AllOrders());
        emit(HomeRegisterSuccessState());
      } else {
        loadingAuthScreens(false);
    ...
    switch (currentItem) {
      case MenuItems.homepage:
       ...
       case MenuItems.profile:
        return const PharmacyProfileUser();
  ...
  void whatsAppOpen(String number) async {
    String url() {
      if (Platform.isIOS) {
        return "whatsapp://wa.me/+2$number/?text=${Uri.encodeFull(
            "مرحبَا، هل يمكنك إخباري بمواعيد العمل الخاصة بك؟")}";
      } else {
        return "whatsapp://send?phone=+2$number&text=${Uri.encodeFull(
            "مرحبَا، هل يمكنك إخباري بمواعيد العمل الخاصة بك؟")}";
      }
    }
  ...
  Future<void> pharmacyOrders() async {
    ...
    var header = {"Accept": "application/json",'Authorization': 'Bearer ${CacheHelper.getData(key: 'token')}'};
    await http.get(uri, headers: header).then((value) {
      if (kDebugMode) {
        print("PharmacyOrders Before is : $value");
      }
      pharmacyOrdersModel = PharmacyOrdersModel.fromJson(jsonDecode(value.body));
      if (kDebugMode) {
...
/*  PharmacyProfileModel? pharmacyProfileModel;
  ...
  Future<void> getMedicines(int page, bool isPagination, String departmentId, String pharmacyId) async {
    ...
    await HttpHelper.getDataWithQuery('pharmacy-medicines', {
      ...
      if (isPagination) {
        pharmacyMedicinesModel!.paginate!.currentPage = value['paginate']['current_page'];
  ...
  Future getCategories() async {
    ...
    await HttpHelper.getData('pharmacy/${CacheHelper.getData(key: "id")}').then((value) {
  ...
  Future<void> finishOrder(String id, BuildContext context,String deliveryPrice, String price) async {
    var url = Uri.https(
      _baseUrlForQuery,
      '${_lastUrlForQuery}order/finish/$id', {
  ...
  Future<void> pharmacyAddMedicines(String price,String amount, BuildContext context) async {
    loadingAuthScreens(true);
  ...
  Future<void> pharmacyEditMedicines(String id,String price,String amount, BuildContext context) async {
    loadingAuthScreens(true);
    emit(PharmacyAddMedicinesLoadingState());
  ...
  Future<void> getPharmacyProfileMedicines(int page, bool isPagination, String pharmacyId) async {
    emit(PharmacyAddMedicinesLoadingState());
    if (isPagination) {
      debugPrint("isPagination = true");
      isPaginationMedicinesLoading = true;
    ...
    await HttpHelper.getDataWithQuery('profile-pharmacy-medicines', {
      ...
      if (isPagination) {
        profileMedicines!.paginate!.currentPage = value['paginate']['current_page'];
        value['paginate']['data'].forEach((e) => profileMedicines!.paginate!.data!.add(prof.Datum.fromJson(e)));
        isPaginationMedicinesLoading = false;
      } else {
